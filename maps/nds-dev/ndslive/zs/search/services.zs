...
struct SupportedSearchConfiguration
{
  ...
  ResultOrder supportedResultOrders
    if (supportedRequestOptions & RequestOptions.RESULT_ORDER) == RequestOptions.RESULT_ORDER;
  ...
  ModuleDefinition poiModuleDefinition
        if (supportedFeatures & SupportedFeature.POI) == SupportedFeature.POI;
  ...
  ModuleDefinition nameModuleDefinition
        if (supportedFeatures & SupportedFeature.ADDRESS) == SupportedFeature.ADDRESS;
...
struct GeneralSearchRequest
{
  ...
  LanguageCode preferredLanguage
    if (options & RequestOptions.PREFERRED_LANGUAGE) == RequestOptions.PREFERRED_LANGUAGE;

  /** Geographic position around which the search is performed. */
  AroundPositionSearchRequestDetails aroundSearchDetails
    if (options & RequestOptions.SEARCH_AROUND) == RequestOptions.SEARCH_AROUND;

  /** Path along which the search is performed. */
  AlongPathSearchRequestDetails alongSearchDetails
    if (options & RequestOptions.SEARCH_ALONG) == RequestOptions.SEARCH_ALONG;
  ...
  SpatialExtent inAreaSearchDetails
    if (options & RequestOptions.SEARCH_IN_GEO_AREA) == RequestOptions.SEARCH_IN_GEO_AREA;

  /** Area defined by ISO country code in which the search is performed. */
  IsoCountryCode isoCountryCode
    if (options & RequestOptions.SEARCH_IN_ISO_COUNTRY) == RequestOptions.SEARCH_IN_ISO_COUNTRY;

  /** Maximum number of responses. */
  varuint maxResponseCount
    if (options & RequestOptions.MAX_RESPONSE) == RequestOptions.MAX_RESPONSE;

  /** Order in which the search results are returned. */
  ResultOrder order
    if (options & RequestOptions.RESULT_ORDER) == RequestOptions.RESULT_ORDER;
  ...
  ExternData poiFilter
    if (options & RequestOptions.POI_FILTER) == RequestOptions.POI_FILTER;
  ...
  ExternData addressFilter
    if (options & RequestOptions.ADDRESS_FILTER) == RequestOptions.ADDRESS_FILTER;
};
...
struct GeocodingAddress
{
  ...
  string addressString
    if (content & GeocodingContentType.ADDRESS_STRING)
               == GeocodingContentType.ADDRESS_STRING;
  /**
  ...
  ExternData addressDetails
    if (content & GeocodingContentType.ADDRESS_DETAILS)
               == GeocodingContentType.ADDRESS_DETAILS;
};
