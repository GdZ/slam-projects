...
irregular pronunciation, phonetic transcription is an integral part of navigation
databases. Navigation systems use phonetic transcriptions for the following purposes:

- Converting text to speech, for example, for route guidance messages or location
  input (Text-to-Speech (TTS))
- Matching the user’s speech input with database items, for example, for location
  input (Automatic Speech Recognition (ASR))

The phonetic transcription list contains a list of phonetic transcriptions and their format.
A phonetic transcription contains the actual transcription string. The transcription
optionally contains information on grammatical case and prefix strings.
