...
application of attributes to a certain time, weather, or other constraining domain.
Conditions are represented as a type and a value.
...
The result is the truth value of the conditions list. For more information, see
[Using the `isInclusive` value](#using-the-isinclusive-value).

**Example**

To determine if the passage on a road is prohibited for a truck, an application
needs to evaluate all `PROHIBITED_PASSAGE` attributes until one of the
following applies:
- At least one attribute has conditions that evaluate to `true` for trucks.
  The prohibition applies.
- All conditions for all `PROHIBITED_PASSAGE` attributes assigned to the road
  evaluate to `false`. None of the `PROHIBITED_PASSAGE` attributes are relevant for
  trucks.
...
prohibited for all vehicles that are not cars than listing all the
vehicles that are prohibited from passing through. Also, the negative
phrase covers any unknown vehicle types.

The `isInclusive` value inverts a condition attribute as described in the
corresponding attribute. The `isInclusive` value is used as follows:

- If `isInclusive` is set to `true`, then the statement expressed by the
  condition value is left unaltered. Example: The condition applies to all buses.
- If `isInclusive` is set to `false`, then the statement expressed by the
  ...
  defined in the condition. Example: The condition applies to all vehicles
  except buses.

### Disjunctive Normal Form of Logical Expressions

NDS.Live uses the disjunctive normal form of logical expressions. The value of a
single condition corresponds to boolean variables as follows:
- A condition with `isInclusive` set to `true` corresponds to an initial boolean variable.
- A condition with `isInclusive` set to `false` corresponds to a negated boolean
variable.
- For bitmasks, the boolean variable corresponds to the relevant bit inside the bitmask.

**Example**

An application finds the following condition for a `PROHIBITED_PASSAGE` attribute:

- `FREQUENTLY_USED_VEHICLE_TYPES`:
    - `FrequentlyUsedVehicleTypes.TRUCK`
    - `isInclusive` = `true`

Because the bit `TRUCK` in the bitmask is set and `isInclusive` is set to
`true`, the prohibition applies to all vehicles that are trucks.

__Note:__
If `isInclusive` is changed to `false`, the prohibition applies to all
vehicles that are not trucks.
...
/** Week in month.
  * Specifies a week in a month. The first week of a month is the one whose
  * Thursday is the first Thursday in that month. Subsequent weeks start on Monday.
...
/** Calendar week.
  * Specifies a calendar weekday according to ISO 8601. Calendar week one
  * is the week with the year's first Thursday. Subsequent calendar weeks
  * start on Monday. The value zero is not valid.
  */
struct CalendarWeek
{
