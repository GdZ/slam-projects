...
- [Positions](#positions)
- [Lines](#lines)
- [Polygons and Meshes](#polygons-and-meshes)
...
The full resolution of NDS coordinates is 31 bit, plus one sign bit.
To statically reduce the resolution to the required level, coordinates can be
shifted to the right using coordinate shift. Before reading, the values have
to be shifted back to the left. Values for coordinate shift are defined once per
geometry layer.
...
of a position buffer in which the most significant byte is never used.
The coordinate width is therefore limited to 20 bits.

![Coordinate shift and coordinate width](assets/geometry-coordinate-shift-width.png)
...
To find the direction, a plane is formed based on the first three
positions of the bounding polygon using the right-hand rule:

- The first position is at the tip of the first finger.
- The second position is the origin of the coordinate system.
- The third position is at the tip of the second finger.
- The surface normal of the resulting plane points in the direction of the thumb.
...
Polygons and meshes are stored in buffers. These buffers comprise positions,
an index buffer, and the actual polygon or mesh.

The polygons and meshes are defined by a range of positions in the index buffer.
Index buffers are used to allow for the re-use and sorting of positions.

The index type determines whether `PolyMesh3D` represents a planar polygon or
a mesh of non-coplanar polygon faces. The following index types can be used:

- `TRIANGLES`: Indices are ordered so that 3 consecutive indices form one
  triangle.
- `TRIANGLE_STRIP`: Every group of 3 adjacent indices forms a triangle. A stream
  of n indices forms n-2 triangles. The first triangle is stored using
...
The following picture shows examples of each of the three index types. The
picture also shows how the triangles are derived from the index in each case.

![Triangle derivation from index](assets/geometry-polygon-index-types.png)
...
{
  /*!
  Winding Order of Triangle Strips:

  If the index type `TRIANGLE_STRIP` is used for a triangulated polygon, the
  positions in the first triangle shall be ordered in counterclockwise winding
  order.
  !*/
...
struct Polygon2D(IndexBuffer ib)
{
  ...
  /** Start index within the index buffer. The index is included. */
  varsize startIndex;

  /** Number of positions from the start that are part of the polygon.
    * `startIndex`+`numPositions` is included and the last position of the polygon.
    */
...
struct PolyMesh3D(IndexBuffer ib, PolyMesh3DType type)
{
  ...
  /** Start index within the index buffer. The index position is included. */
  varsize startIndex;
...
counterclockwise winding order. The line forming the ring does not intersect
itself.
...
- The positions in outer rings are stored in counterclockwise winding order.
- The positions in inner rings are stored in clockwise winding order.
...
inner ring. The positions in the outer or inner rings are ordered
in counterclockwise or clockwise winding order, respectively.
...
{
  /*!
  Winding Order of Line Rings in Simple Polygons:

  The positions within each `Ring2D` structure used in `SimplePolygon2D`
  shall be ordered in counterclockwise winding order.
  !*/

  rule "core-kf2f9t";

  /*!
  Winding Order of Outer Rings in Simple Polygon Sets:
  ...
  /*!
  Winding Order of Inner Rings in Simple Polygon Sets:

  The positions within each `Ring2D` structure used in `SimplePolygonSet2D.holes`
  shall be ordered in clockwise winding order.
  !*/
