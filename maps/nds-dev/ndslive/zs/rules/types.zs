...
struct RegionDrivingRules
{
  ...
  bool stayCurbside if (content & DrivingRulesContent.STAY_CURBSIDE) == DrivingRulesContent.STAY_CURBSIDE;

  /** Color of temporary lane markings that overrule existing markings. */
  MarkingColor temporaryLaneMarkingColor
          if (content & DrivingRulesContent.TEMPORARY_LANE_MARKING_COLOR) == DrivingRulesContent.TEMPORARY_LANE_MARKING_COLOR;

  /** Defines whether passing on a lane closer to the curb is allowed. */
  bool curbsidePassingAllowed
          if (content & DrivingRulesContent.CURBSIDE_PASSING) == DrivingRulesContent.CURBSIDE_PASSING;

  /** Rules for emergencies. */
  EmergencyRules emergencyRules
          if (content & DrivingRulesContent.EMERGENCY_RULES) == DrivingRulesContent.EMERGENCY_RULES;
  ...
  /**
    ...
    */
  bool yieldOncoming
          if (content & DrivingRulesContent.YIELD_RULES) == DrivingRulesContent.YIELD_RULES;
  ...
  bool alwaysAllowed
          if (content & DrivingRulesContent.SOLID_LINE_RULES) == DrivingRulesContent.SOLID_LINE_RULES;

  /** Rules for when solid lines may be crossed. */
  SolidLineCrossingRules solidLineCrossingRules
      if (content & DrivingRulesContent.SOLID_LINE_RULES) == DrivingRulesContent.SOLID_LINE_RULES && !alwaysAllowed;

  /** Rules for on-street parking. */
  RoadSideParkingRules roadSideParkingRules
      if (content & DrivingRulesContent.ROADSIDE_PARKING_RULES) == DrivingRulesContent.ROADSIDE_PARKING_RULES;

  /** Rules for lane merge situations. */
  LaneMergeRules laneMergeRules
      if (content & DrivingRulesContent.LANE_MERGE_RULES) == DrivingRulesContent.LANE_MERGE_RULES;
};
...
/**
  ...
  * TODO: Create a rule for this one?
  * TODO: refactor this kind of modeling?
  */
...
struct TimeZoneConfiguration
{
     ...
     * The time is given as an offset from UTC (Universal Time Coordinated).
     */
    DayOfYear validFromDate;

    /** Time offset in quarter hour units relative to UTC. */
