...
for example, its type, width, color, or material. The boundary elements are
defined once and are reused using their identifier. Boundary elements have
different types:
...
The boundary element details are used to precisely define additional properties
...
/**
  ...
  * further properties like, for example, its width, color, or material.
  */
...
Boundary sets order parts of lane boundaries sequentially along the lane. Lane
boundaries do not need to be cut to fit the ranges of boundary sets.

A boundary set is valid from its start position (included) on the lane center
line to the start position of the next boundary set (excluded). The last
boundary set in the list of boundaries is valid until the end of the lane.
...
- If boundary geometries are available, boundary sets contain directed
  references to lane boundaries that are ordered from lane center to the outside
  of the lane. A boundary set contains one reference position for each parallel
  boundary element inside the set. The position indicates where the boundary set
  starts on the boundary geometry, regardless of the digitization direction of
  the boundary geometry.
- If no boundary geometries are available, boundary sets contain references to
  boundary elements instead. The boundary sets are cut each time that a boundary
...
rule_group BoundarySetRules
{
  ...
  /*!
  Only One Physical Divider per Boundary Geometry:

  If a boundary geometry in a boundary set is referenced by a boundary element
  of type `PHYSICAL_DIVIDER`, then this geometry shall not additionally be
  referenced by any other boundary element of type `PHYSICAL_DIVIDER`,
  `PHYSICAL_MARKING`, or `MARKING`. The number of boundary elements ot type
  `LOGICAL` that reference this boundary geometry is not limited.
  !*/
...
/**
  * Set of lane boundary parts that are ordered sequentially along a lane.
...
{
  /**
    * Position on the lane where the boundary set starts. The set is valid from
    * this start position (included) to the start of the next boundary
    * set (excluded).
    */
  LaneGeometryPosition startPosition;

  /**
    * References to detailed lane boundary element mappings ordered from inside
    * to outside of the lane if boundary geometries are available.
    * The `BoundaryReferenceWithPosition.position` indicates where the boundary
    * set starts on the boundary geometry.
    */
