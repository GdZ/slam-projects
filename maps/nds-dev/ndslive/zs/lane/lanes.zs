...
struct Lane(LaneLayoutType layout, bool hasBoundaryGeometry)
{
  ...
  /** Sets of lane boundaries on the left side of the lane. */
  BoundarySet(hasBoundaryGeometry) boundariesLeft[];

  /** Sets of lane boundaries on the right side of the lane. */
...
rule_group ZeroLengthLaneRules
{
  /*!
  No Attribution on Zero-Length Lanes:

  A zero-length lane shall not have any attributes assigned to it.
  !*/
...
- If it is known that the lane ends, the type `TERMINATES` is used.
...
enum uint8 LaneRelationType
{
  ...
  /** The related lane's surface overlaps with the current lane's surface. */
  OVERLAP,
};
