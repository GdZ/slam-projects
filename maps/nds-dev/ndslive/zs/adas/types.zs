...
In contrast to the positions of a road geometry, ADAS geometry is stored with
ADAS precision as needed by the attributes. The ADAS geometry is independent of
the road geometry and can contain different positions than the geometry of a
road.

The following figure illustrates the difference between ADAS geometries and
regular road geometries. In this example, the ADAS geometry has more positions
...
rule_group AdasGeometryRules
{
  ...
  Every `AdasGeometryPosition` used within ADAS road attributes shall refer to
  the `AdasGeometry` that is associated with the same road.
  !*/
  ...
  Every `AdasGeometryPosition` used within ADAS lane attributes shall refer to
  the geometry of the associated lane.
  !*/

  rule "adas-03ufw8";
};

/** List of ADAS geometries. */
...
/**
  * Position on the ADAS geometry if used with roads.
  * Position on the lane geometry if used with lanes.
  */
...
Comfortable speed values can be assigned to transitions, parts of roads, or a
path. The comfortable speed path is described by a sequence of directed roads.
`ComfortableSpeedPath` is assigned to the first road of this sequence.
...
using a sequence of points with slope values. Each curvature point in a
...
/** Comfortable speed. */
subtype Speed ComfortableSpeed;
