...
struct NameString
{
    ...
    uint8          refNameStringIndex if nameStringRelationType != NameStringRelationType.NO_RELATION;
};
...
electric recharging point, charging point, EVSE (Electric Vehicle Supply Equipment),
and EVCE (Electric Vehicle Charging Equipment). An EV charging station supplies
electricity for the charging of electric vehicles and other plug-in vehicles,
including plug-in hybrids.

An EV charging station consists of a number of chargers, also called charging spots.
These chargers usually have one or more different connectors. Depending on the
connector type, different levels of power are provided.

The EV charging station details model the available chargers based on the
connector type because this is the most important information for a vehicle.

The availability information for an EV charging station provides details about
how many chargers of each connector type are available, occupied, reserved, or
out of service.
...
{
  /**
    * Total power provided for all connectors.
    * The total power can be less than the sum of the maximum power values of
    * all connectors.
    */
  Watt totalPower;

  /** Number of different connector types at the EV charging station. */
  varsize numConnectors;

  /** List of different connector types at the EV charging station. */
  EvConnector connectors[numConnectors];

  /** Number of chargers per connector type. */
  varuint16 chargers[numConnectors];
};

/** Availability information for an EV charging station. */
struct EvChargingAvailability
{
  ...
  EvConnectorType connectorType[numConnectors];

  /** Number of available/free chargers for a certain connector type. */
  varuint16 chargersAvailable[numConnectors];

  /** Number of occupied chargers for a certain connector type. */
  varuint16 chargersOccupied[numConnectors];

  /** Number of reserved chargers for a certain connector type. */
  varuint16 chargersReserved[numConnectors];

  /** Number of chargers that are out of service for a certain connector type. */
  varuint16 chargersOutOfService[numConnectors];

};

/** Definition of an EV charging connector type. */
struct EvConnector
{
  /** Type of connector. */
  EvConnectorType type;

  /** Electric current of the connector. */
  ElectricCurrent current;

  /** Maximum power in Watt supplied by the connector. */
  Watt maxPower;

  /** Voltage range of the connector. */
  optional Voltage voltage;

  /** Ampere range of the connector. */
  optional Ampere ampere;
...
/** Available EV communication standards. */
